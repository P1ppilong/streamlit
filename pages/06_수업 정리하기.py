...
if st.session_state.login == '':
    ...
    with tab1:
        ...
        # -----------------------------
        if st.button("채점하기"):
            mcq_answers = {"q1": "② 엘니뇨는 해수 온도가 높아지고, 라니냐는 해수 온도가 낮아진다.", "q2": "② 지역별로 다른 형태의 이상기후를 유발한다.", "q3": "② 엘니뇨 기간 동안 기온이 평년보다 상승하는 경향을 보인다."}
            score = 0
            wrong = []
            ...
            st.write("### 4번 예시 답안")
            st.write("- 라니냐는 일부 지역의 강수량을 증가시킬 수 있으며, 이로 인해 라니냐 기간 동안 평균 강수량이 증가한 지역에서는 홍수나 폭우 같은 현상이 발생했음을 알 수 있다.")

            st.write("### 5번 예시 답안")
            st.write("엘니뇨 지수와 기온은 강한 양의 상관관계를 가진다. 엘니뇨가 강할수록 기온이 상승하는 경향이 있다.")

            st.write("### 6번 예시 답안")
            st.write("이는 지구 온난화로 인해 해수 온도의 변동성이 커졌음을 의미하며, 앞으로 극단적 기후현상이 더욱 빈번해질 가능성을 시사한다.")
